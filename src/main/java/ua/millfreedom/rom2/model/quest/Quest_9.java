package ua.millfreedom.rom2.model.quest;

/**
 * Native class: Quest_9.
 */
public class Quest_9 extends Quest {
    /**
     * Native: Quest_9::New @005482F0.
     * Fully ported.
     */
    public Quest_9() {
        super();
    }

    /**
     * vtbl +0x28: Quest_9::getId @00548310.
     * Fully ported.
     */
    @Override
    public int getId() {
        return 9;
    }

    /**
     * vtbl +0x2C: Quest_9::getEventMessageCode @00548320.
     * Fully ported.
     */
    @Override
    public int getEventMessageCode() {
        return MESSAGE_RAISE_DEAD_9_EVENT;
    }

    /**
     * vtbl +0x30: Quest_9::getEventMessageArgument @00548330.
     * Fully ported.
     */
    @Override
    public int getEventMessageArgument() {
        return ownerPlayerId | (secondaryArgument << 16);
    }

    /**
     * vtbl +0x14: Quest_9::OnQuestMessage @0052F2D4.
     * Fully ported.
     */
    @Override
    public int onQuestMessage(int message, int arg2, int arg3) {
        if (message == MESSAGE_RAISE_DEAD_9_EVENT
                && secondaryArgument <= lowWord(arg2)
                && lowWord(arg3) == ownerPlayerId
                && highWord(arg3) == secondaryIndexKey
                && !isCompleted()) {
            int spellPower = arg2 >>> 16;
            tertiaryArgument = spellPower * spellPower / 5;
            storage.addQuestRelatedEntry(MESSAGE_MARK_ACTIVE, questKey, 0);
        }
        return super.onQuestMessage(message, arg2, arg3);
    }
}
