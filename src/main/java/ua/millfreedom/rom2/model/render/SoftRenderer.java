...
//        }
     ...
     */
    protected static A16RunWriter getA16CompositingRunWriter(int surfaceLeft, int surfaceTop, int pitch, ByteBuffer surface, Palette16[] palettePages) {
     ...
     */
    protected static A16RunWriter getA16BasePaletteCompositingRunWriter(int surfaceLeft, int surfaceTop, int pitch, ByteBuffer surface, RGB16[] palette16) {
        return (x, y, encodedPixels, offset, count, stepX) -> {
            int di = (y - surfaceTop) * pitch + (x - surfaceLeft) * 4;
            ByteBuffer bb = ByteBuffer.wrap(encodedPixels)
                    .order(ByteOrder.LITTLE_ENDIAN)
     ...
     * not ported.
     */
    protected static Rle8RunWriter getRle8To32RunWriter(RGB16[] palette16, int pitch, int clipTop, int clipLeft, ByteBuffer surface, boolean blend) {
     ...
     */
    protected static Rle8RunWriter getRle8ShadeRunWriter(int shadePage, int pitch, int clipTop, int clipLeft, ByteBuffer surface) {
        return (x, y, paletteIndices, offset, count, stepX) -> {
            int di = (y - clipTop) * pitch + (x - clipLeft) * 4;
            for (int i = 0; i < count; i++) {
                RGB32 destinationColor = new RGB32(surface.getInt(di));
     ...
     */
    protected static Rle8RunWriter getRle8LutRunWriter(int lutIndex, int pitch, int clipTop, int clipLeft, ByteBuffer surface) {
        return (x, y, paletteIndices, offset, count, stepX) -> {
            int di = (y - clipTop) * pitch + (x - clipLeft) * 4;
            for (int i = 0; i < count; i++) {
                RGB32 destinationColor = new RGB32(surface.getInt(di));
     ...
     */
    protected static void putTerrainPixel(ByteBuffer surface, int destinationOffset, byte[] sourcePixels,
    ...
    // not ported.
    protected static void put32LE(ByteBuffer surface, int off, RGB32 rgb32) {
        if (off < 0 || off + 3 >= surface.limit()) return;
        surface.putInt(off, rgb32.val());
    }

    // not ported.
    protected static void put32LE(ByteBuffer surface, int off, RGB16 rgb16) {
        if (off < 0 || off + 3 >= surface.limit()) return;
     ...
     */
    protected static void put32Additive(ByteBuffer surface, int off, RGB16 src) {
        if (off < 0 || off + 3 >= surface.limit()) return;
     ...
     */
    protected static void put32ShadeAdditive(ByteBuffer surface, int off, int shade, RGB16 additiveColor) {
        if (off < 0 || off + 3 >= surface.limit()) return;
     ...
     */
    protected static RGB16 nativeShadeAdditiveColor(int shade) {
     ...
     */
    protected static void put32Blend(ByteBuffer surface, int off, RGB32 src) {
        if (off < 0 || off + 3 >= surface.limit()) return;
    ...
    // not ported.
    protected static void put32Clipped(
            ByteBuffer surface,
            int pitchBytes,
            int surfaceLeft,
            int surfaceTop,
    ...
    // not ported.
    protected static void putAtXY(ByteBuffer surface, int pitchBytes, int left, int top, int x, int y, short rgb565) {
        int sx = x - left;
        int sy = y - top;
        int off = sy * pitchBytes + sx * 4; // 32bpp
        if (off < 0 || off + 3 >= surface.limit()) return;
